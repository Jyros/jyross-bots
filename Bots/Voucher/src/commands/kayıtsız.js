...
module.exports = {
conf: {
aliases: ["kytsz","unreg","unregister","unregistered"],
name: "kayıtsız",
},
...
jyros.roles.set([settings.roles.unregistered]);
...
client.guilds.cache.get(config.guildID).channels.cache.get(settings.channels.logs.kayıtsızLog).send(embed.setDescription(`${jyros} (\`${jyros.id}\`) adlı kullanıcı ${message.author} (\`${message.author.id}\`) tarafından kayıtsıza atıldı.`));
}}