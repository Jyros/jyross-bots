...
if (!jyros){message.channel.send(embed.setDescription(`Eksik Argüman! \`${config.PREFIX}e @Jyros/ID\``).setColor(`${config.embed.color.red}`)).then(mal => mal.delete({timeout: 7000})); message.react(settings.emojis.no);}
...
ydb.add(`erkekTeyit.${message.author.id}`, 1);
...
jyros.roles.add(settings.roles.manRoles);
...
client.guilds.cache.get(config.guildID).channels.cache.get(settings.channels.logs.kayıtErkekLog).send(embed.setColor(`${config.embed.color.white}`).setTitle(`Bir Üye Kayıt Edildi!`).setDescription(`
Üye: ${jyros} (\`${jyros.id}\`)
